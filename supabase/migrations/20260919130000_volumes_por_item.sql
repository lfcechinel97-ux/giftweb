-- =====================================================================
-- GIFT WEB - Volumes da expedição passam a ser POR ITEM, não por
-- pedido inteiro.
--
-- Motivo (feedback do usuário): "o volume... possivelmente só vão
-- ficar atrelada a um produto, e não a todos... às vezes em uma caixa
-- consigo colocar 3 produtos" -- uma caixa não necessariamente carrega
-- o pedido inteiro. sistema_pedidos.volumes (pedido inteiro) fica
-- como estava, sem uso neste fluxo novo; sistema_producao_itens.volumes
-- guarda a caixa específica que aquele item foi embalado, e itens que
-- foram na MESMA caixa reaproveitam o mesmo valor via "Já preenchido"
-- em vez de preencher nos formulário de novo.
-- =====================================================================

alter table public.sistema_producao_itens
  add column if not exists volumes jsonb;
  -- formato: {"responsavel": text, "itens": [{"comprimento":cm,"altura":cm,"largura":cm,"peso":kg}, ...]}
  -- nota fiscal/etiqueta de envio continuam na tabela de anexos
  -- (sistema_producao_anexos, categorias 'nota_fiscal'/'etiqueta') --
  -- já existente, sem coluna nova pra isso.
