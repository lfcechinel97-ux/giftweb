...
CREATE INDEX IF NOT EXISTS idx_producao_itens_tags ON public.sistema_producao_itens USING gin (tags);

CREATE TABLE IF NOT EXISTS public.sistema_producao_comentarios (
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.sistema_producao_comentarios TO authenticated;
GRANT ALL ON public.sistema_producao_comentarios TO service_role;

ALTER TABLE public.sistema_producao_comentarios ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Autenticados leem comentarios de producao"
  ON public.sistema_producao_comentarios FOR SELECT TO authenticated USING (true);

CREATE POLICY "Autenticados criam comentarios de producao"
  ON public.sistema_producao_comentarios FOR INSERT TO authenticated WITH CHECK (auth.uid() = autor_id OR autor_id IS NULL);

CREATE POLICY "Autor apaga o proprio comentario"
  ON public.sistema_producao_comentarios FOR DELETE TO authenticated USING (auth.uid() = autor_id);

CREATE INDEX IF NOT EXISTS idx_producao_comentarios_item
  ON public.sistema_producao_comentarios (producao_item_id, created_at);
...
CREATE OR REPLACE VIEW public.vw_pcp AS
  SELECT pi.id AS producao_id,
     ...
     item.value ->> 'observacao'::text AS item_observacao,
