  ...
  return (
    <section className="py-10 md:py-14 bg-background">
      ...
      >
        ...
        >
          {cats.map((cat) => (
            <Link key={cat.key} to={cat.route} className="flex flex-col items-center gap-3 group">
              <div
...
function MobileCarousel({ cats }: { cats: Array<{ key: string; name: string; img: string; route: string }> }) {
  ...
  for (let i = 0; i < cats.length; i += 2) {
    if (i + 1 < cats.length) groups.push([cats[i], cats[i + 1]]);
    else groups.push([cats[i], cats[0]]);
  }
  ...
  useEffect(() => {
    const t = setInterval(() => goTo((slide + 1) % total), 2500);
  ...
  return (
    <div className="lg:hidden overflow-hidden relative">
      ...
      >
        {current.map((cat, i) => (
          <Link key={`${slide}-${i}`} to={cat.route} className="flex flex-col items-center gap-2.5 no-underline">
            <div className="w-[120px] h-[120px] rounded-full overflow-hidden border-2 border-border bg-muted" style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
