...
const Header = () => {
  ...
  return (
    <>
      ...
      >
        {/* Top bar — azul escuro */}
        <div className="bg-navy text-white/70" style={{ fontSize: 13 }}>
        ...
        {/* Main bar — fundo branco */}
        <div
          className="border-b border-border transition-all duration-300 bg-white"
          style={{
        ...
        >
          <div className="container flex items-center justify-between gap-4 py-3">
            <a href="/" className="flex items-baseline gap-0.5 shrink-0 min-w-0">
              <span className="font-extrabold font-serif text-left text-3xl sm:text-4xl md:text-5xl text-navy leading-none">
                Gift Web
              </span>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="hidden sm:flex items-center gap-2 rounded-[10px] bg-green-cta px-5 py-2.5 text-sm font-bold text-navy hover:brightness-110 transition-all duration-200 shrink-0"
            ...
            <button
              ...
              className="lg:hidden text-navy p-1 z-[110] relative"
            >
              {mobileOpen ? <X size={24} /> : <Menu size={24} />}
            </button>
          </div>

          {/* Category nav — desktop, fundo branco */}
          <div className="hidden lg:block border-t border-border bg-white">
            <div className="container flex items-center gap-0 py-0">
              <a
                href="/produtos"
                className="flex items-center gap-2 bg-navy text-white px-5 py-3 rounded-[8px] my-1 text-[13px] font-bold uppercase hover:bg-navy-hover transition-colors duration-200 shrink-0"
              >
                <Menu size={16} />
                TODOS BRINDES
              </a>
              {categoryLinks.map((cat) => (
                <a
                  key={cat.name}
                  href={cat.route}
                  className="px-4 py-3 text-[13px] font-bold uppercase text-navy hover:text-green-cta transition-colors duration-200"
                >
                  {cat.name}
                </a>
              ))}
      ...
      >
        ...
        <nav className="px-6 flex flex-col gap-1 overflow-y-auto flex-1 pb-10">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-6 flex items-center justify-center gap-3 rounded-2xl bg-green-cta px-6 py-4 text-sm font-bold text-navy"
            style={{ boxShadow: "0 0 24px rgba(163,230,53,0.35)" }}
          >
