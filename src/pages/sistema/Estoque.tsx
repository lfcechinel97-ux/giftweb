...
// ─── Aba: Estoque XBZ (consulta, todos os produtos + variantes) ───────────────
function EstoqueXBZ({ search, categoria }: { search: string; categoria: string }) {
  ...
  }, [parentProducts, variantesByParent, search, categoria]);
          ...
          return (
            <div key={p.id} className="bg-card border border-border rounded-lg overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-4 py-2.5">
                ...
                {p.categoria && <Badge variant="secondary" className="text-[9px] h-4 shrink-0">{p.categoria}</Badge>}
                <div className="shrink-0 text-right min-w-[90px]">
                  <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border ${m.color}`}>
              ...
              {pvars.map(v => {
                ...
                return (
                  <div key={v.id} className="flex items-center gap-3 px-4 py-1.5 bg-muted/10 border-t border-dashed border-border text-xs">
                    ...
                    <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border ${vm.color}`}>
...
// ─── Aba: Estoque Próprio (produtos reservados em orçamentos aprovados) ────────
function EstoqueProprio({ search, categoria }: { search: string; categoria: string }) {
  const { parentProducts, variants, isLoading } = useSistemaProducts();
  const { ajustesEstoque } = useSistema();

  // Soma de todos os ajustes negativos (reservas) por produtoId
  ...
  // Mostra apenas produtos que têm ajuste de reserva > 0, ou toda a lista filtrada
  const filtered = useMemo(() => {
    const term = search.trim().toLowerCase();
    return parentProducts.filter(p => {
      if (categoria !== "todas" && p.categoria !== categoria) return false;
  ...
  const comReserva = useMemo(
    ...
    [filtered, variantesByParent, ajustesEstoque]
  );
              ...
              <div key={p.id} className="bg-card border border-border rounded-lg overflow-hidden">
                <div className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <div className="shrink-0 text-right space-y-0.5">
                    <div className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border ${m.color}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${m.dot}`} />
                      {disponivel} disponível
                    </div>
                    {ajuste > 0 && (
                      <p className="text-[10px] text-red-600 font-medium">−{ajuste} reservado</p>
                    )}
                  </div>
                </div>
                {pvars.filter(v => getReserva(v.id, v.codigo_amigavel) > 0).map(v => {
