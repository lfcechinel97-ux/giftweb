        ...
        <main className="flex-1">
          <div className="container py-6 md:py-8">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {products.map((p) => (
                  <ProductCard key={p.id} id={p.id} nome={p.nome} slug={p.slug} image_url={p.image_url} image_urls={p.image_urls} cor={p.cor} preco_custo={p.preco_custo} codigo_amigavel={p.codigo_amigavel} variantes={p.variantes as any} estoque={p.estoque} estoque_total={p.estoque_total} tabela_precos={p.tabela_precos} />
                ))}
              </div>
            )}
