...
DIR_ICONES = RAIZ / "data" / "story_icons"   # PNG com fundo recortado (transborda o circulo)
...
# Nome curto para o rotulo do story (o nome completo da categoria nao cabe)
ROTULO_CURTO = {
    "COPOS, GARRAFAS E CANECAS": "Copos e Garrafas",
    "MOCHILAS, BOLSAS TÉRMICAS E MALAS": "Mochilas e Bolsas",
    "CADERNETAS, AGENDAS, BLOCOS E CANETAS": "Cadernetas",
    "CAIXAS DE SOM, FONES E POWER BANK": "Som e Eletrônicos",
    "KIT CHURRASCO E KIT VINHO": "Churrasco e Vinho",
    "NECESSAIRES, PORTA JOIAS E KIT MANICURE": "Necessaires",
    "CHAVEIROS, MOUSE PAD E KIT EXECUTIVO": "Escritório",
    "MARMITAS E TÁBUAS DE MADEIRA": "Marmitas e Tábuas",
    "SACOLA DE ALGODÃO E TNT": "Sacolas",
...
def montar_html(produtos: list[dict], categorias: list[str]) -> str:
    ...
    cats_meta = []
    for c in categorias:
        icone = DIR_ICONES / f"{slug(c)}.png"
        cats_meta.append({
            "nome": c, "curto": ROTULO_CURTO.get(c, c.title()), "slug": slug(c),
            "img": b64(icone) if icone.exists()
                   else next(p["img"] for p in produtos if p["c"] == c),
            "png": icone.exists(),
        })

    dados = json.dumps({"produtos": produtos, "categorias": cats_meta, "destaques": destaques,
                        "whatsapp": WHATSAPP, "qtdIni": QTD_INICIAL, "passo": QTD_PASSO,
                        "atalhos": list(QTD_ATALHOS)},
    ...
    return TEMPLATE.replace("__DADOS__", dados) \
                   ...
                   .replace("__INTER__", b64(DIR_FONTS / "inter-latin.woff2")) \
                   .replace("__SORA__", b64(DIR_FONTS / "sora-latin.woff2")) \
...
:root{
  ...
  --h:56px;--r:14px;--sh:0 1px 3px rgba(9,36,60,.08),0 6px 18px rgba(9,36,60,.06);
}
...
h1,h2,h3{font-family:'Sora','Inter',system-ui,sans-serif;letter-spacing:-.02em}
...
/* STORIES - imagem transborda o circulo para ganhar enfase */
.stwrap{position:relative}
.stories{display:flex;gap:10px;overflow-x:auto;padding:26px 12px 10px;scrollbar-width:none;
  scroll-snap-type:x proximity;-webkit-overflow-scrolling:touch}
.stories::-webkit-scrollbar{display:none}
.st{flex:none;width:104px;text-align:center;scroll-snap-align:center;background:none}
.st .circ{position:relative;width:92px;height:92px;margin:0 auto 9px}
.st .circ::before{content:'';position:absolute;inset:0;border-radius:50%;background:#e8edf4;
...
.st span{font-size:11.5px;line-height:1.3;color:var(--ink);font-weight:500;display:block}
.st.on .circ::before{background:#d3e0f0;box-shadow:0 0 0 2.5px var(--green-500)}
.stfade{position:absolute;top:0;bottom:0;right:0;width:36px;pointer-events:none;
  background:linear-gradient(90deg,rgba(238,242,247,0),var(--paper))}
...
.chips button{flex:1;height:26px;border-radius:7px;background:var(--paper);border:1px solid var(--line);
  color:var(--navy-600);font-size:11px;font-weight:600;transition:.15s}
...
.qty button{width:32px;height:30px;display:grid;place-items:center;color:var(--navy-600);font-size:17px;font-weight:600;border-radius:7px}
.qty button:active{background:var(--paper)}
.qty .qv{font-size:13.5px;font-weight:600;font-variant-numeric:tabular-nums;min-width:4ch;text-align:center}
.qty .qv i{font-style:normal;font-size:10px;color:var(--muted);font-weight:500}
...
.fab .fc{background:#fff;color:var(--green-600);min-width:24px;height:24px;border-radius:12px;
  display:grid;place-items:center;font-size:12.5px;font-weight:700;padding:0 6px}
...
.ci .q span{font-size:12px;font-weight:600;min-width:3ch;text-align:center;font-variant-numeric:tabular-nums}
...
.resumo b{font-family:'Sora',sans-serif;font-size:19px;color:var(--navy-800);font-weight:700}
...
.obh h2{margin:0 0 5px;font-size:19px;font-weight:700}
...
.stepn{flex:none;width:32px;height:32px;border-radius:50%;background:var(--paper);color:var(--navy-700);
  display:grid;place-items:center;font-family:'Sora',sans-serif;font-weight:700;font-size:14px}
.step .sc{flex:1}
.step h3{margin:0 0 3px;font-size:13.5px;font-weight:600;color:var(--navy-800);font-family:'Inter',sans-serif}
.step p{margin:0;font-size:12px;color:var(--ink-2);line-height:1.5}
.step .mini{display:inline-flex;align-items:center;gap:4px;background:var(--paper);border:1px solid var(--line);
  border-radius:6px;padding:2px 7px;font-size:11px;font-weight:600;color:var(--navy-700);margin-top:6px}
...
.obbtn{width:100%;height:48px;border-radius:12px;background:var(--green-600);color:#fff;
...
footer{text-align:center;padding:30px 20px calc(96px + env(safe-area-inset-bottom));color:var(--muted);font-size:11.5px;line-height:1.7}
...
<main>
  <div class="bwrap">
    <div class="btrack" id="btrack">
      ...
      <div class="bslide b2">
        <h2>Peça a partir de 10 unidades</h2>
        <p>Monte sua lista e finalize direto no WhatsApp com um consultor.</p>
      </div>
    </div>
...
<button class="fab zero" id="fab" aria-label="Abrir carrinho">
  ...
  <span class="fl">Meu pedido</span>
  <span class="fc" id="fabCount">0</span>
</button>
...
/* Carrinho: array em memoria. Sem localStorage/sessionStorage de proposito -
   ...
   Nao guarda preco: o valor exibido e "a partir de", entao o fechamento do
   valor real acontece com o consultor no WhatsApp. */
var carrinho = [];
...
function cardHTML(p){
  var chips = ATALHOS.map(function(v){
    return '<button data-act="set" data-v="'+v+'">'+v+' un</button>';
  }).join('');
...
function lerQtd(card){
  return parseInt(card.querySelector('.qv').textContent, 10) || QTD_INI;
}
...
function montar(){
  ...
  $('#stories').innerHTML = CATEGORIAS.map(function(c){
    return '<button class="st" data-cat="'+c.slug+'">'
      + '<div class="circ"><img src="data:image/'+(c.png?'png':'jpeg')+';base64,'+c.img+'" alt=""></div>'
      + '<span>'+esc(c.curto)+'</span></button>';
  }).join('');
  ...
  CATEGORIAS.forEach(function(c, i){
    ...
    html += '<section class="catsec" id="cat-'+c.slug+'" data-cat="'+c.slug+'">'
      + '<div class="head"><h2>'+esc(c.curto)+'</h2>'
  ...
  if(!carrinho.length){
    ...
    box.innerHTML = carrinho.map(function(i){
      return '<div class="ci" data-id="'+i.id+'">'
        ...
        + '<button class="rm" data-cact="rm">remover</button></div>'
        + '</div></div>';
    }).join('');
  }
...
document.addEventListener('click', function(e){
  ...
  if(s){
    document.querySelectorAll('.st').forEach(function(x){ x.classList.toggle('on', x === s); });
...
/* dica de rolagem nos stories: pequeno "empurrao" ao abrir */
function dicaStories(){
  var s = $('#stories');
  if(s.scrollWidth <= s.clientWidth + 8) return;
  setTimeout(function(){
    s.scrollTo({ left: 64, behavior: 'smooth' });
    setTimeout(function(){ s.scrollTo({ left: 0, behavior: 'smooth' }); }, 620);
  }, 700);
}
